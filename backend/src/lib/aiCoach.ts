...
export function buildCoachSystemPrompt(opts: {
  ...
  return [
    ...
    `- Pergunta uma coisa de cada vez. Nunca despejes tres perguntas seguidas.`,
    `- Quando o utilizador descreve uma situacao, faz 1 ou 2 perguntas para clarificar e DEPOIS propoe a regra.`,
    `- Antes de criar uma regra, valida em texto natural com o admin: "Entao a regra fica assim: X. Confirmas?".`,
    `- Se o utilizador disser "sim", "ok", "podes guardar" ou equivalente, cria a regra na mesma resposta (no array actions).`,
    `- Sugere proactivamente categorias e keywords baseadas na situacao.`,
    ...
    ``,
    `Regras ja gravadas neste workspace:`,
    rulesBlock,
    ``,
    `Formato OBRIGATORIO da resposta: JSON com esta forma exacta, sem markdown, sem texto antes ou depois:`,
    `{`,
    `  "reply": "<o que vais dizer ao admin, PT-MZ, sem travessoes, 1 a 6 frases curtas>",`,
    `  "actions": [`,
